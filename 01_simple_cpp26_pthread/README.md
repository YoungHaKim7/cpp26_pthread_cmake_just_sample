...
clang := `which clang-21` 
clangpp := `which clang++-21` 
gpp := `which g++`
...
# Common flags
ldflags_common := "-std=c++2c -pthread -lm -Wall -Wextra -ggdb -Werror"
...
# highway
include_highway := "-I /opt/homebrew/Cellar/highway/1.3.0/include"
lib_highway := "-L /opt/homebrew/Cellar/highway/1.3.0/lib -lhwy"

# (C)gpp compile(LinuxOS)
r:
	rm -rf {{target_dir}}
	mkdir -p {{target_dir}}
	{{gpp_which}} {{ldflags_common}} {{include_highway}} {{lib_highway}} -o {{target_dir}}/{{project_name}} {{source}}
	{{target}}
...
mr:
    rm -rf {{target_dir}}
    export CXX={{gpp_which}}
    CXX={{gpp_which}} meson setup {{target_dir}}
